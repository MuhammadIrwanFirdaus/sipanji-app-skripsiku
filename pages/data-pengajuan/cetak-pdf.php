...
function createPDF($start_date, $end_date) {
    ...
    $selectSql = "SELECT * FROM data_pengajuan WHERE tgl_masuk BETWEEN :start_date AND :end_date";
    ...
    $pdf->Cell(0, 10, 'Data Pengajuan', 0, 1, 'C');
...
foreach ($pengajuan as $event) {
    ...
    $indoDate = indoDate($event['tgl_masuk']); // Menggunakan fungsi indoDate yang telah didefinisikan sebelumnya
    $pdf->Cell(80, 10, $indoDate, 1, 0, 'L');
    $pdf->Cell(60, 10, $event['tempat'], 1, 0, 'L');
    $pdf->Cell(60, 10, $event['kategori'], 1, 1, 'L');
...
$pdf->SetX(10);
    ...
    $pdf->Output('Laporan Data Pengajuan.pdf', 'D');
    exit();
}
